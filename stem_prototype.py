...
for dataset in datasets:
    adj_path = f'/mnt/sdb/yujia/DCRNN_PyTorch/matrix/{dataset}_{no_intervals_all}_{agg_type}.pkl'
    # adj = load_graph_data(adj_path)
    # mul_L = torch.tensor(adj).unsqueeze(0)
    ...
    # result_fold = './result'
    result_train_fold = './result1/train'
    result_test_fold = './result1/test'
    ...
    class GLU(nn.Module):
        ...
        def forward(self, x):
            # print('GLU model shape', x.shape, self.linear_left(x).shape, self.linear_right(x).shape)
            # torch.Size([32, 26, 60]) torch.Size([32, 26, 60]) torch.Size([32, 26, 60])
    ...
    class StockBlockLayer(nn.Module):
        def __init__(self, time_step, unit, multi_layer, stack_cnt=0):
            ...
            self.output_channel = self.multi
            for i in range(3):
                if i == 0:
                #     self.GLUs.append(GLU(self.time_step * 4, self.time_step * self.output_channel))
                #     self.GLUs.append(GLU(self.time_step * 4, self.time_step * self.output_channel))
                    self.GLUs.append(GLU(self.time_step * 1, self.time_step * self.output_channel))
                    self.GLUs.append(GLU(self.time_step * 1, self.time_step * self.output_channel))
        ...
        def spe_seq_cell(self, input):
            ...
            for i in range(3):
                # print('real before GLU', real.shape) # 32 26 12 torch.Size([32, 26, 12])
            ...
            real = real.reshape(batch_size, node_cnt, 1, -1).permute(0, 2, 1, 3).contiguous()
            img = img.reshape(batch_size, node_cnt, 1, -1).permute(0, 2, 1, 3).contiguous()
        ...
        def forward(self, x, mul_L):
            ...
            # print('StockBlockLayer Check: \n mul_L shape, x shape\n', mul_L.shape, x.shape)
            #torch.Size([1, 1, 26, 26]) torch.Size([32, 1, 1, 26, 12])

            gfted = torch.matmul(mul_L, x)

            # print('gfted shape', gfted.shape)
            # gfted shape torch.Size([32, 1, 1, 26, 12])
    ...
    class Model(nn.Module):
        ...
        def forward(self, x):
            ...
            #input size torch.Size([32, 12, 26])

            # mul_L, attention = self.latent_correlation_layer(x)
            
            mul_L = torch.tensor(load_graph_data(adj_path)).float().unsqueeze(0).to(device)
            # print('mul_L size', mul_L.shape)
            # mul_L size torch.Size([1, 26, 26])
            # print('attention after latent', attention.shape)

            X = x.unsqueeze(1).permute(0, 1, 3, 2).contiguous().float()
            #(batch, sequence, features) ==> (batch, 1, features, sequences)
            result = []
            # print('X_Shape', X.shape, 'mul_L shape', mul_L.shape)
            # X_Shape torch.Size([32, 1, 26, 12]) mul_L shape torch.Size([1, 26, 26])
    ...
    def test(test_data, 
        ...
        performance_metrics['mae_node_mean'] = mae_node
        performance_metrics['mape_node_mean'] = mape_node
        performance_metrics['rmse_node_mean'] = rmse_node

        # write_json(performance_metrics, f'/mnt/sdb/yujia/pmf/result1/test/{dataset}_{no_intervals_all}_{agg_type}/performance_metrics.json')
        pd.DataFrame(performance_metrics).to_csv(f'/mnt/sdb/yujia/pmf/result1/test/{dataset}_{no_intervals_all}_{agg_type}/performance_metrics.csv', index = False)
